...
public class TableMetadataTest extends CCMTestsSupport {
  ...
  public void should_parse_compact_dynamic_table() {
    ...
    String cql =
        String.format(
            "CREATE TABLE %s.compact_dynamic (\n"
                + "    k text,\n"
                + "    c int,\n"
                + "    v timeuuid,\n"
                + "    PRIMARY KEY (k, c)\n"
  ...
  public void should_parse_compact_table_with_multiple_clustering_columns() {
    ...
    String cql =
        String.format(
            "CREATE TABLE %s.compact_composite (\n"
                + "    k text,\n"
                + "    c1 int,\n"
                + "    c2 float,\n"
                + "    c3 double,\n"
  ...
  public void should_parse_table_options() {
    ...
    if (isRealCassandra4) {
      ...
      // With 2.1 we have different options, the caching option changes and replicate_on_write
      // disappears
      assertThat(table.getOptions().getReadRepairChance()).isEqualTo(0.5);
      assertThat(table.getOptions().getLocalReadRepairChance()).isEqualTo(0.6);
      assertThat(table.getOptions().getGcGraceInSeconds()).isEqualTo(42);
  ...
  /**
   * Validates that metadata is appropriately parsed when using the new compression options format
   * introduced in 3.0.0-alpha1. Since compression options are parsed as a String map, this should
   * behave exactly as the old style compression options used in {@link
   * #should_parse_table_options}.
   *
  ...
  public void should_parse_new_compression_options() {
    ...
    String cql =
        String.format(
            "CREATE TABLE %s.new_compression_options (\n"
                + "    k text,\n"
                + "    c1 int,\n"
                + "    c2 int,\n"
                + "    i int,\n"
    ...
    session().execute(cql);
    TableMetadata table =
        cluster().getMetadata().getKeyspace(keyspace).getTable("new_compression_options");
  ...
  /**
   * Validates that the 'extensions' option is properly parsed when set on a table. This value is
   * currently not modifiable via CQL so we fake out a table containing populated extensions by
   * updating the extensions column in system_schema.tables and forcing a schema refresh on it.
   *
  ...
  public void should_parse_extensions_from_table_options() throws Exception {
    ...
    String cql =
        String.format(
            "CREATE TABLE %s.table_with_extensions (\n"
                + "    k text,\n"
                + "    c int,\n"
                + "    v timeuuid,\n"
                + "    PRIMARY KEY (k, c)\n"
    ...
    // Manually change column value in system_schema.tables and force a schema refresh on that
    // table.
    ImmutableMap<String, ByteBuffer> extensions =
        ImmutableMap.of("Hello", ByteBuffer.wrap("World".getBytes("UTF-8")));
    session()
        .execute(
    ...
    cluster()
        .manager
        .controlConnection
        .refreshSchema(SchemaElement.TABLE, keyspace, "table_with_extensions", null);

    // when retrieving the table's metadata.
    TableMetadata table =
        cluster().getMetadata().getKeyspace(keyspace).getTable("table_with_extensions");
  ...
  /**
   * Validates that a table with case-sensitive column names and column names consisting of (quoted)
   * reserved keywords is correctly parsed and that the generated CQL is valid.
   *
