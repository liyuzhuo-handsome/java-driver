...
public abstract class TestUtils {
  ...
  public static void setValue(
      SettableByNameData<?> data, String name, DataType type, Object value) {
    switch (type.getName()) {
      case ASCII:
  ...
  public static Object getValue(
      GettableByIndexData data, int i, DataType type, CodecRegistry codecRegistry) {
    switch (type.getName()) {
      case ASCII:
  ...
  public static Object getValue(
      GettableByNameData data, String name, DataType type, CodecRegistry codecRegistry) {
    switch (type.getName()) {
      case ASCII:
      ...
      switch (type.getName()) {
        ...
        case MAP:
          return new HashMap<Object, Object>() {
            {
              put(
                  getFixedValue2(type.getTypeArguments().get(0)),
   ...
   *
   * <p>Primitive types are defined as the types that don't have type arguments (that is excluding
   * lists, sets, and maps, tuples and udts).
   *
  ...
  /**
   * Returns the IP of the {@code nth} host in the cluster (counting from 1, i.e., {@code
   * ipOfNode(1)} returns the IP of the first node.
   *
  ...
  /**
   * Returns the address of the {@code nth} host in the cluster (counting from 1, i.e., {@code
   * ipOfNode(1)} returns the address of the first node.
   *
   * <p>In multi-DC setups, nodes are numbered in ascending order of their datacenter number. E.g.
   * with 2 DCs and 3 nodes in each DC, the first node in DC 2 is number 4.
   *
  ...
  public static Host findOrWaitForControlConnection(
      final Cluster cluster, long duration, TimeUnit unit) {
   ...
   * This is useful for tests that consistently close clusters as otherwise there is a 2 second
   * delay (from JAVA-914).
   */
   ...
   *
   * <p>Any of params given as an Integer will be considered with a field name of 'i', any as String
   * will be considered with a field name of 's'.
   *
  ...
  public static ByteBuffer serializeForDynamicCompositeType(Object... params) {
    ...
    for (ByteBuffer bb : l) res.put(bb);
    res.flip();
    return res;
  }
   ...
   *
   * <p>Expects Integer and String types for parameters.
   *
   * @param params params to serialize.
